...
- **`sync` post-merge reconciliation**: distinguishes a squash-merged/landed branch from a genuinely divergent one, instead of dead-ending both as `rebase conflict — aborted`. **Reactive**: only when a rebase conflicts, `sync` runs `git merge-tree --write-tree <target> <branch>` (in-memory 3-way merge, no index lock — parallel-safe) and treats the branch as **LANDED** iff it exits 0 AND its result tree OID equals `<target>^{tree}` (`is_landed` in `commands.rs`; primitives in `git.rs`). LANDED surfaces a new `~ reconcilable` third state (distinct from `ok`/`ERR`). **Comparison target** (`resolve_sync_target`) is a safe-degrading chain: `{remote}/{effective_upstream}` → on gone-after-prune fall back to `{remote}/{default_branch}` (`upstream_gone`) → else non-fatal skip. Classify runs in the parallel worker pool; **remediation is a deferred sequential pass** (`remediate_reconcilables`) because the spinner can't prompt mid-flight. Consent: per-repo `[y/N]` in a TTY, `--reconcile` for non-interactive/CI, detect-and-hint (never mutate) when piped. Remediation = `git reset --hard <target>` (content-safe by tree-equality, re-verified at reset for TOCTOU; clean worktree required, `--stash` the only override; pre-reset SHA printed for reflog); gone-upstream additionally clears the `.rig.json` `upstream` field (the durable fix — `sync` reads the manifest, not git tracking config). Continue-and-report; genuine divergence keeps `ERR (rebase conflict — aborted)`. Requires git ≥ 2.38 (`merge-tree --write-tree`), which is why the global floor bumped 2.30 → 2.38. Spec: `docs/brainstorms/2026-07-13-post-merge-reconciliation.md`.
...
- **`doctor` command**: Two-tier health check — environment (git on PATH, git version >= 2.38) then per-repo (reuses `check_drift()` for worktree/source/branch checks, adds origin/HEAD, remote reachability, upstream branch existence). Works outside a rig (env checks only) or inside (full checks). PASS/WARN/FAIL output with copy-paste fix commands. Exit 1 on any issue. Environment failures short-circuit per-repo checks.
